...
// floating point
void draw_affine_texel(int x, int y, vec2_t a, vec2_t b, vec2_t c, tex2_t a_uv,
					   tex2_t b_uv, tex2_t c_uv, uint32_t *texture);

// Draw texel same as above but passing in extra information for perspective
// correct interpolation
void draw_texel(int x, int y, vec4_t a, vec4_t b, vec4_t c, tex2_t a_uv,
				tex2_t b_uv, tex2_t c_uv, uint32_t *texture);
