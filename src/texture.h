...
// Draw texel based not only on xy coords of point, but also it's interpolated uv coords from triangle vertice info
// We pass in x and y as int so that the we draw at the right discrete pixel, everything else is calculated using floating point
void draw_affine_texel(int x, int y, vec2_t a, vec2_t b, vec2_t c, tex2_t a_uv, tex2_t b_uv, tex2_t c_uv, uint32_t* texture);

// Draw texel same as above but passing in extra information for perspective correct interpolation
