...
void load_obj_file_data(const char *file_name) {
	...
	while (fgets(line, 512, obj_file)) {
		...
		if (line[0] == 'v' && line[1] == ' ') {
			vec3_t obj_vertex;
			if (!sscanf(line, "v %f %f %f", &obj_vertex.x, &obj_vertex.y,
						&obj_vertex.z)) {
			...
			if (!sscanf(line, "vt %f %f", &coord.u, &coord.v)) {
				fprintf(
					stderr,
					"Error reading vertex texture coordinate data .obj file");
			}
			coord.v = 1 - coord.v; // adjust because .obj files have an inverted
								   // v compared to the renderer
		...
		// Faces
			...
			if (!sscanf(
				...
				fprintf(stderr,
						"Error reading vertex indice data from .obj file");
			}

			face_t obj_face = {
				.a = vertex_indices[0] -
					 1, // adjust by 1, indices start at 1 in .obj format
