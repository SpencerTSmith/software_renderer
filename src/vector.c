...
vec3_t vec3_rotate_x(vec3_t v, float angle) {
	vec3_t result = {v.x, v.y * cosf(angle) - v.z * sinf(angle),
					 v.y * sinf(angle) + v.z * cosf(angle)};
	return result;
}

vec3_t vec3_rotate_y(vec3_t v, float angle) {
	vec3_t result = {v.x * cosf(angle) - v.z * sinf(angle), v.y,
					 v.x * sinf(angle) + v.z * cosf(angle)};
	return result;
}

vec3_t vec3_rotate_z(vec3_t v, float angle) {
	vec3_t result = {v.x * cosf(angle) - v.y * sinf(angle),
					 v.x * sinf(angle) + v.y * cosf(angle), v.z};
	return result;
}
